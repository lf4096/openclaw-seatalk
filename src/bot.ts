...
async function processBufferedDmEvents(
	...
	if (accessDecision.decision !== "allow") {
		if (accessDecision.reason === "not allowlisted") {
			...
			log(
				`seatalk[${accountId}]: blocked DM from ${employeeCode} (${
					accessDecision.reason ?? "denied"
				})`,
			);
		}
		return;
	...
	try {
		...
		const typingResult = core.channel.reply.createReplyDispatcherWithTyping({
			...
			deliver: async (payload) => {
				...
				if (reply.hasText) {
					log(
						`seatalk[${accountId}]: inline deliver DM to ${employeeCode} threadId=${
							threadId || "none"
						}`,
					);
		...
		try {
			...
			log(
				`seatalk[${accountId}]: dispatch complete (queuedFinal=${queuedFinal}, counts=${JSON.stringify(
					counts,
				)})`,
			);
		} finally {
...
export async function handleSeaTalkMessage(params: {
	...
	log(
		`seatalk[${accountId}]: received ${msgEvent.message.tag} from ${
			msgEvent.employee_code
		} (threadId=${msgEvent.message.thread_id || "none"})`,
	);
...
function checkGroupAccess(params: {
	...
	if (groupPolicy === "allowlist") {
		...
		if (!list.includes(groupId)) {
			return {
				allowed: false,
				reason: `group ${groupId} not in groupAllowFrom`,
			};
		}
	}
...
async function processBufferedGroupEvents(
	...
	try {
		...
		try {
			...
			log(
				`seatalk[${accountId}]: group dispatch complete (queuedFinal=${queuedFinal}, counts=${JSON.stringify(
					counts,
				)})`,
			);
		} finally {
